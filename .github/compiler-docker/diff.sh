...
for new in head/sources/*.gpkg; do
    ...
    if ! cmp -s "$old" "$new"; then
        ...
        if grep -q "\"type\": \"LineString\"" "$oldJ" || grep -q "\"type\": \"MultiLineString\"" "$oldJ" ; then
            ...
            qgis_process run native:buffer --distance_units=meters --area_units=m2 --ellipsoid=EPSG:7030 "--INPUT=$oldJ.unbuf.geojson" "--DISTANCE=0.01" --SEGMENTS=5 --END_CAP_STYLE=0 --JOIN_STYLE=0 --MITER_LIMIT=2 --DISSOLVE=false "--OUTPUT=$oldJ"
            qgis_process run native:buffer --distance_units=meters --area_units=m2 --ellipsoid=EPSG:7030 "--INPUT=$newJ.unbuf.geojson" "--DISTANCE=0.01" --SEGMENTS=5 --END_CAP_STYLE=0 --JOIN_STYLE=0 --MITER_LIMIT=2 --DISSOLVE=false "--OUTPUT=$newJ"
        fi
